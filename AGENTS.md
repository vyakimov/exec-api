...
- `EXEC_API_READ_PREFIXES` — required colon-separated list of absolute path prefixes that `/read-file` is allowed to read under. Resolved at startup with symlinks followed. The server refuses to start if unset.
...
- `/read-file` resolves the requested path (following symlinks) and rejects anything not under `EXEC_API_READ_PREFIXES`. Size is capped at 10 MiB.
